...
namespace Flux.Core.Objects {
	public class BlockStorage {
		...
		public byte[] Write() {
			MinecraftStream stream = new MinecraftStream();
			stream.WriteByte((byte) BitsPerEntry);
			stream.WriteVarInt(States.Count);
			foreach (int state in States) stream.WriteVarInt(state);

			long[] data = Storage.GetData();
			stream.WriteVarInt(data.Length);

			foreach (long i in data) stream.WriteLong(i);

			for (int i = 0; i < 16 * 16 * 16 / 2; i++) stream.WriteByte(255);
		...
		public int[] GetStates() => States.ToArray();

		public FlexibleStorage GetStorage() => Storage;
